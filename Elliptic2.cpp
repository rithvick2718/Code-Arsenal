...
bool is_primitive(pair<int,int> xy, const int &a, const int &p,int grp_size, map< pair<int,int> , int > &mp){
    if(mp[xy] == -1) return false;
    auto [x,y] = xy;
    pair<int,int> id = {-1,-1};
    auto val = xy;
...
pair<int,int> find_primitive_element(const vector<pair<int,int>> &group_elements,const int &a, const int &p){
    ...
    map< pair<int,int> , int > mp; // by default it's 0, -1 means it's not a primitive root , +1 means it is!
    for(auto p1 : group_elements){
        if(is_primitive(p1,a,p,grp_size,mp)) return p1;
    }
    return {-1,-1};
}
int main(){
